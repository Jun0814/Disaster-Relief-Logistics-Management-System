...
private:
    ...
    void loadFromCSV()
    {
        ...
        while (getline(file, line))
        {
            ...
            if (supply.supplyID >= nextID)
                nextID = supply.supplyID + 1;
        }
        file.close();
    }
...
public:
    ...
    void viewPendingSupplyRequests()
    {
        ...
        while (getline(file, line))
        {
            ...
            if (status == "Pending")
            {
                string type = supplyID == "1" ? "food" : supplyID == "2" ? "beverage"
                                                     : supplyID == "3"   ? "clothes"
                                                                         : "other";
                cout << left
                     << setw(8) << reqID
        ...
        if (!found)
            cout << "No pending supply requests.\n";
        cout << "-------------------------------------------------------------\n";
        file.close();
    }
